...
FakeSnapshot = collections.namedtuple(
    'FakeSnapshot', ('name', 'resource_name')
)
...
class MultiLinstor(object):
    ...
    def snapshot_dfn_list_raise(self, filter_by_nodes=None,
                                filter_by_resources=None):
        return FakeSnapshotList([
            FakeSnapshot(snapname, rscname)
            for rscname, rsc in self.__resources.items()
            if not filter_by_resources or rscname in filter_by_resources
            for snapname in rsc.get('snapshots', {})
        ])
...
class Resource(object):
    ...
    def clone(self, clone_name, use_zfs_clone=None):
        if clone_name in self.__resources:
            raise LinstorError()

        self.__resources[clone_name] = {
            'nodes': self.existing['nodes'],
            'volumes': self.volumes,
            'allow_two_primaries': self.allow_two_primaries,
        }
        return Resource(
