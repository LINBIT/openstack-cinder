...
linstor_opts = [
    ...
    cfg.IntOpt('linstor_redundancy',
               deprecated_name='linstor_autoplace_count',
               help='Deploy a replica on at least the specified number of '
                    'nodes. If not set, use inherited value from resource '
                    'group. A value of 0 is invalid.'),
...
class LinstorDriver(driver.VolumeDriver):
    ...
    * A Cinder Volume Type maps to a Resource Group in Linstor.
    ...
    def check_for_setup_error(self):
        ...
        if self.configuration.linstor_redundancy == 0:
            msg = _('Using redundancy of 0 for deploying on every replica is'
                    'no longer supported')
            raise LinstorDriverException(msg)
...
class LinstorDirectTarget(targets.Target):
    ...
    def __init__(self, client, *args, **kwargs):
        ...
        """
        super(LinstorDirectTarget, self).__init__(*args, **kwargs)
        self.c = client
...
class LinstorDriverException(exception.VolumeDriverException):
    def __init__(self, message=None, **kwargs):
        super(LinstorDriverException, self).__init__(message, **kwargs)


class LinstorDriverApiException(exception.VolumeBackendAPIException):
    def __init__(self, error, **kwargs):
        super(LinstorDriverApiException, self).__init__(data=error, **kwargs)
