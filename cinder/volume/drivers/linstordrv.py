    ...
    * A Cinder Volume Type maps to a Resource Group in Linstor.
    ...
    def _init_vendor_properties(self):
        ...
        self._set_property(
            ...
            'linstor:layer_list',
            title='Layer List',
            description='Comma-separated list of layers to apply for '
                        'resources. If empty, defaults to DRBD,Storage.',
            type='str',
        )
        self._set_property(
            self._vendor_properties,
    ...
    def delete_volume(self, volume):
        ...
        except LinstorVolumeNotFoundException as e:
            LOG.debug(
                'resource %s already deleted, ignoring: %s',
                volume['name'],
                e
            )
            return
    ...
    def delete_snapshot(self, snapshot):
        ...
        with self.c.get() as lclient:
            existing_names = _existing_snapshot_names(lclient, rsc)

        # Snapshots created by the v1 driver use the "SN_<id>" naming scheme
        candidate_names = [snapshot['name'], 'SN_' + snapshot['id']]
        matching_names = [n for n in candidate_names if n in existing_names]
        if not matching_names:
            LOG.warning('Snapshot %s of volume %s not found in LINSTOR, '
                        'assuming it was already deleted',
                        snapshot['name'], rsc.name)

        for name in matching_names:
            try:
                rsc.snapshot_delete(name)
            except linstor.LinstorError:
                raise exception.SnapshotIsBusy(name)
    ...
    def terminate_connection(self, volume, connector, **kwargs):
        """Terminate a connection, unless the host still uses the volume

        Multi-attach volumes can be attached multiple times from the same
        host. Like the LVM driver, the connection is only terminated once the
        last attachment from that host is gone.
        :param volume: The volume to detach
        :param dict|None connector: The connector to detach. None on a force
         detach, in which case every connection is terminated.
        :return: True, if other connections to the volume remain
        :rtype: bool
        """
        attachments = volume['volume_attachment']

        if connector is None:
            self.target_driver.terminate_connection(volume, None, **kwargs)
            return False

        if volume['multiattach']:
            same_host_attachments = [
                a for a in attachments
                if a.connector and self.target_driver.are_same_connector(
                    a.connector, connector,
                )
            ]
            if len(same_host_attachments) > 1:
                return True
...
def _get_existing_resource(linstor_client, volume_name, volume_id):
    ...
    raise LinstorVolumeNotFoundException(volume_name)


def _existing_snapshot_names(linstor_client, resource):
    """Get the names of all snapshots LINSTOR knows for a resource

    :param linstor.Linstor linstor_client: Client used for API calls
    :param linstor.Resource resource: The resource to list snapshots for
    :return: The snapshot names
    :rtype: set[str]
    """
    snapshot_list = linstor_client.snapshot_dfn_list_raise(
        filter_by_resources=[resource.name],
    )
    return {snap.name for snap in snapshot_list.snapshots}
...
class LinstorVolumeNotFoundException(LinstorDriverException):
    def __init__(self, volume_name, **kwargs):
